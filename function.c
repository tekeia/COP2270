...
int main ()
{
    ...
    for (i=0;i<3;i++)
    {
      ...
      charge[i] = calculateCharges(cust[1]);
    ...
    for(i=0;i<3;i++)
    {
      ...
      number of hours and serial nuber of the car. */
      printf("%d\t%0.1f\t%0.2f\n", i+1,cust[i],charge[i]);;
    }
    printf("Total\t%.1f\t%.2f\n",toth,totc);
}
float calculateCharges(float hours)
{
  ...
  if (hours>3)
  {
    if(hours>=19)
    {
      /*Assign 10 to the charge if the number of hours
      is greater than or equal to 19.*/
      charge=10;
    }
    else
    {
      /*Calculate the charges if the number of hours
      is greater than 3 and less than 19.*/
      charge=2+ceil(hours-3)*0.5;
    }
  }
  else
  {
    /*Assign 2  to the charge if the number of hours is
